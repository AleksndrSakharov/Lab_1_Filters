...
namespace WindowsFormsApp1
{
    ...
    class WaveFilter : Filters
    {
        ...
        {
            double x = k + 20.0 * Math.Sin(2 * Math.PI * k / 30.0);
            double y = l;
            Color sourceColor = sourceImage.GetPixel(Clamp((int)x, 0, sourceImage.Width), (int)y);

            return Color.FromArgb(Clamp(sourceColor.R, 0, 255), Clamp(sourceColor.G, 0, 255), Clamp(sourceColor.B, 0, 255)); ;
        }
    }
}
